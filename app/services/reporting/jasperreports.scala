...
import net.sf.jasperreports.engine.data.{JRXmlDataSource, JRBeanArrayDataSource}
import java.util
...
trait JRXmlReportCompiler extends ReportCompiler {
  ...
  def compileReport(in: In): ReportT[JasperReport] = {
    println("report compileReport")
    for {
      loaded <- load(in)
      input <- EitherT.fromTryCatch[Id, JasperDesign] {
        println(s"jrxml load $in")
        JRXmlLoader.load(loaded)
      }
...
trait ReportRunner {
  ...
  private def eval(expression: Expression): ReportT[ExpressionValue] = {
    ...
    expression match {
      ...
      case e => {
        println("bad match in eval ")
        EitherT.left[Id, Throwable, ExpressionValue](BadExpressionException(e))
      }
    }
  }
  ...
  def runReportT(in: In)
    ...
    for {
      ...
      out <- EitherT.fromTryCatch[Id, Array[Byte]] {
        println("format")
        format(root, parameters, dataSource)
      }
    } yield out
  }
